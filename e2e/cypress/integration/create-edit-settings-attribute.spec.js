...
describe("Create & edit attribute", function () {
    ...
    describe("Create", () => {
        ...
        it("saved the attribute", () => {
            ...
            cy.get('input[data-cy=personal-info-chkbx').invoke('prop', 'checked').then(checked => expect(checked).to.be.true)

            cy.get('input[data-cy=translation-long').should('have.value', mockText)
            cy.get('input[data-cy=translation-short').should('have.value', mockText)
