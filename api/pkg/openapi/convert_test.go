...
func TestStructuralRoundtripOrError(t *testing.T) {
	...
	f.Funcs(
		func(s *core.JSON, c fuzz.Continue) {
			m := map[string]interface{}{"foo": float64(42.2)}
			b, _ := json.Marshal(m)
			*s = core.JSON{Raw: b}
		},
		func(s *core.JSONSchemaPropsOrArray, c fuzz.Continue) {
			c.FuzzNoCustom(s)
			if s.Schema != nil {
				s.JSONSchemas = nil
			} else if s.JSONSchemas == nil {
				s.Schema = &core.JSONSchemaProps{}
			}
		},
		func(s *core.JSONSchemaPropsOrBool, c fuzz.Continue) {
			c.FuzzNoCustom(s)
			if s.Schema != nil {
				s.Allows = false
			}
	...
	for i := 0; i < 10000; i++ {
		...
		origSchema := &core.JSONSchemaProps{}
		...
		v1Schema := &corev1.JSONSchemaProps{}
		err = json.Unmarshal([]byte(bs), v1Schema)
		if err != nil {
			t.Fatal(err)
		}
		internalSchema := &core.JSONSchemaProps{}
		err = corev1.Convert_v1_JSONSchemaProps_To_core_JSONSchemaProps(v1Schema, internalSchema, nil)
		if err != nil {
			t.Fatal(err)
		}
