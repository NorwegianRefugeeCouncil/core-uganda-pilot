...
func (s *MainTestSuite) TestFormDefinitionPostValidationErrors() {
	...
	for _, testCase := range testCases {
		...
		s.T().Run(tc.name, func(t *testing.T) {
			...
			out, err := s.nrcClient.CoreV1().FormDefinitions().Create(s.ctx, fd)
			if tc.assert != nil {
				tc.assert(t, out, err)
			}
			if err == nil {
				// deleting the entity if it was created (cleanup)
				assert.NoError(t, s.nrcClient.CoreV1().FormDefinitions().Delete(s.ctx, out.GetUID(), metav1.DeleteOptions{}))
			}
		})
	}
