...
it('should convert a form and record to the correct format', () => {
  ...
  expect(buildDefaultFormValues([form], [record])).toEqual({
    [form.id]: {
...
it('should convert multiple forms and records to the correct format', () => {
  ...
  expect(buildDefaultFormValues([form1, form2], [record1, record2])).toEqual({
    [form1.id]: {
...
it('should throw an error if the forms and records do not align', () => {
  const form = makeForm(1);
  const record = makeRecord(2);
  expect(() => buildDefaultFormValues([form], [record])).toThrowError();
});
