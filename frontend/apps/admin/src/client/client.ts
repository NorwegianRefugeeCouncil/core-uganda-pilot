...
export default class Client implements ApiClient {
  ...
  listIdentityProviders(request: IdentityProviderListRequest): Promise<IdentityProviderListResponse> {
    return this.do(
      request,
      `${this.address}/${this.adminv1}/identityproviders?organizationId=${request.organizationId}`,
      'get',
      undefined,
      200,
    );
  ...
  getSession(request: void): Promise<SessionGetResponse> {
    return this.do(request, `${this.address}/${this.adminv1}/oidc/session`, 'get', undefined, 200, {
      headers: {},
    });
  }
}
