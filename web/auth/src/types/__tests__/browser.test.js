...
describe('utils/browser', () => {
    ...
    describe('openAuthSessionAsync', () => {
        ...
        let windowSessionSetItemMock = jest.fn(() => 'handle');
        ...
        it('should call functions', async () => {
            const openAuthSessionAsyncPromise = browser.openAuthSessionAsync({url: 'https://localhost:8080'})
    ...
    describe('getStateFromUrlOrGenerateAsync', () => {
        ...
        it('should return generateStateAsync when no state in url', async () => {
            const result = await browser.getStateFromUrlOrGenerateAsync('http://www.google.com')
            expect(result).toEqual('generatedState')
            expect(spy).toHaveBeenCalledTimes(1);
        })

        it('should return state if present in url', async () => {
            const result = await browser.getStateFromUrlOrGenerateAsync('http://www.google.com?state=state')
    ...
    describe('generateStateAsync', () => {
        ...
        it('should throw error when crypto.subtle is not available', async () => {
            ...
            await expect(browser.generateStateAsync())
                .rejects
                .toThrow('ERR_WEB_BROWSER_CRYPTO: The current environment does not support Crypto');
        })
    ...
    describe('generateRandom', () => {
        ...
        let getRandomValuesSpy = jest.fn(() => 'randomValues');
