...
describe('utils/helpers', () => {
    ...
    describe('listener', () => {
        ...
        it('should resolve', () => {
            const event = {data: {sender: browser.getHandle()}, isTrusted: true, origin: 'http://localhost'};
        ...
        it('should return if not trusted', () => {
            const event = {data: {sender: browser.getHandle()}, isTrusted: false, origin: 'http://localhost'};
            listener(event, browser, resolveMock);
            expect(resolveMock).not.toHaveBeenCalled()
        })

        it('should return if origin not correct', () => {
            const event = {data: {sender: browser.getHandle()}, isTrusted: true, origin: 'http://randomOrigin'};
            listener(event, browser, resolveMock);
            expect(resolveMock).not.toHaveBeenCalled()
        })
    })
