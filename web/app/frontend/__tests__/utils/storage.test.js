...
import {deleteEncryptedLocalData, getEncryptedLocalData, storeEncryptedLocalData} from '../../src/utils/storage'
...
describe("utils/storage", () => {
    ...
    describe(storeEncryptedLocalData.name, () => {
        it('should call internal fns with the correct params', async () => {
            ...
            expect(SecureStore.setItemAsync).toHaveBeenCalledWith(recordId, key);
            expect(CryptoJS.AES.encrypt).toHaveBeenCalledWith(JSON.stringify(data), key);
            expect(AsyncStorage.setItem).toHaveBeenCalledWith(recordId, encryptedData);
        })
    })
