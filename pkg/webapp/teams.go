...
func (s *Server) Teams(w http.ResponseWriter, req *http.Request) {
	...
	t, err := iamClient.Teams().List(ctx, iam2.TeamListOptions{})
	if err != nil {
		s.Error(w, err)
		return
	}
...
func (s *Server) Team(w http.ResponseWriter, req *http.Request) {
	...
	m, err := iamClient.Memberships().List(ctx, iam2.MembershipListOptions{
		TeamID: id,
	})
	if err != nil {
		s.Error(w, err)
	...
	n, err := iamClient.Nationalities().List(ctx, iam2.NationalityListOptions{
		TeamID: id,
	})
	if err != nil {
		s.Error(w, err)
...
func (s *Server) AddIndividualToTeam(w http.ResponseWriter, req *http.Request) {
	...
	_, err = iamClient.Memberships().Create(ctx, &iam2.Membership{
		ID:           "",
		TeamID:       t,
		IndividualID: i,
	})
