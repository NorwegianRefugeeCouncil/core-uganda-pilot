...
func (s *Server) dangerouslySetAuthenticatedUserUsingEmail(w http.ResponseWriter, req *http.Request, authUserEmail string, handler http.Handler) {
	...
	var profile = &Claims{}
	profile.Email = authUser.Get(iam2.EMailAttribute.ID)
	profile.DisplayName = authUser.Get(iam2.DisplayNameAttribute.ID)
	profile.FullName = authUser.Get(iam2.FullNameAttribute.ID)
