...
func (s *MainTestSuite) TestFormDefinitionCRUD() {
	...
	watcher, err := s.nrcClient.CoreV1().FormDefinitions().Watch(watchCtx, v1.ListOptions{})
	...
	out, err := s.nrcClient.CoreV1().FormDefinitions().Create(s.ctx, &formDefinition)
	if err != nil {
	...
	updated, err := s.nrcClient.CoreV1().FormDefinitions().Update(s.ctx, out)
	...
	assert.NoError(t, s.nrcClient.CoreV1().FormDefinitions().Delete(s.ctx, out.GetUID(), v1.DeleteOptions{}))
