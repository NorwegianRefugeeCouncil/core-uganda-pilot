...
import (
	...
	"github.com/nrc-no/core/apps/api/pkg/tools/pager"
	"github.com/nrc-no/core/apps/api/pkg/util/exceptions"
	"github.com/nrc-no/core/apps/api/pkg/util/trace"
...
func DefaultWatchErrorHandler(r *Reflector, err error) {
	...
	case isExpiredError(err):
		...
		logrus.Infof("%s: watch of %v closed with: %v", r.name, r.expectedTypeName, err)
	case err == io.EOF:
		// watch closed normally
	case err == io.ErrUnexpectedEOF:
		logrus.Infof("%s: Watch for %v closed with unexpected EOF: %v", r.name, r.expectedTypeName, err)
	default:
		logrus.Errorf("%s: Failed to watch %v: %v", r.name, r.expectedTypeName, err)
	}
}
...
func (r *Reflector) setExpectedType(expectedType interface{}) {
	...
	if obj, ok := expectedType.(*unstructured.Unstructured); ok {
		...
		if gvk.Empty() {
			logrus.Infof("Reflector from %s configured with expectedType of *unstructured.Unstructured with empty GroupVersionKind.", r.name)
			return
		}
		r.expectedGVK = &gvk
...
func (r *Reflector) Run(stopCh <-chan struct{}) {
	logrus.Infof("Starting reflector %s (%s) from %s", r.expectedTypeName, r.resyncPeriod, r.name)
	wait.BackoffUntil(func() {
		if err := r.ListAndWatch(stopCh); err != nil {
			r.watchErrorHandler(r, err)
		}
	}, r.backoffManager, true, stopCh)
	logrus.Infof("Stopping reflector %s (%s) from %s", r.expectedTypeName, r.resyncPeriod, r.name)
}
...
func (r *Reflector) ListAndWatch(stopCh <-chan struct{}) error {
	logrus.Infof("Listing and watching %v from %s", r.expectedTypeName, r.name)
	...
	go func() {
		...
		for {
			...
			if r.ShouldResync == nil || r.ShouldResync() {
				logrus.Infof("%s: forcing resync", r.name)
	...
	for {
		...
		if err != nil {
			...
			if utilnet.IsConnectionRefused(err) || exceptions.IsTooManyRequests(err) {
		...
		if err := r.watchHandler(start, w, &resourceVersion, resyncerrc, stopCh); err != nil {
			if err != errorStopRequested {
				...
				case exceptions.IsTooManyRequests(err):
					logrus.Infof("%s: watch of %v returned 429 - backing off", r.name, r.expectedTypeName)
					<-r.initConnBackoffManager.Backoff().C()
					continue
				default:
					logrus.Warningf("%s: watch of %v ended with: %v", r.name, r.expectedTypeName, err)
				}
			}
			return nil
		}
...
loop:
	for {
		...
		case event, ok := <-w.ResultChan():
			...
			if event.Type == watch.Error {
				return exceptions.FromObject(event.Object)
			}
			if r.expectedType != nil {
				if e, a := r.expectedType, reflect.TypeOf(event.Object); e != a {
					logrus.Errorf("%s: expected type %v, but watch event object had type %v", r.name, e, a)
					continue
				}
			}
			if r.expectedGVK != nil {
				if e, a := *r.expectedGVK, event.Object.GetObjectKind().GroupVersionKind(); e != a {
					logrus.Errorf("%s: expected gvk %v, but watch event object had gvk %v", r.name, e, a)
					continue
				}
			}
			meta, err := meta.Accessor(event.Object)
			if err != nil {
				logrus.Errorf("%s: unable to understand watch event %#v", r.name, event)
				continue
			}
			...
			case watch.Added:
				...
				if err != nil {
					logrus.Errorf("%s: unable to add watch event object (%#v) to store: %v", r.name, event.Object, err)
				}
			case watch.Modified:
				err := r.store.Update(event.Object)
				if err != nil {
					logrus.Errorf("%s: unable to update watch event object (%#v) to store: %v", r.name, event.Object, err)
				}
			case watch.Deleted:
				...
				if err != nil {
					logrus.Errorf("%s: unable to delete watch event object (%#v) from store: %v", r.name, event.Object, err)
				}
			case watch.Bookmark:
	...
	logrus.Infof("%s: Watch close - %v total %v items received", r.name, r.expectedTypeName, eventCount)
	return nil
}
...
func isExpiredError(err error) bool {
	...
	return exceptions.IsResourceExpired(err) || exceptions.IsGone(err)
}

func isTooLargeResourceVersionError(err error) bool {
	if exceptions.HasStatusCause(err, metav1.CauseTypeResourceVersionTooLarge) {
		return true
	}
	...
	if !exceptions.IsTimeout(err) {
		return false
	}
	apierr, ok := err.(exceptions.APIStatus)
