...
func GetObjectMeta(obj map[string]interface{}, dropMalformedFields bool) (*metav1.ObjectMeta, bool, error) {
  ...
  for k, v := range metadataMap {
    ...
    if singleFieldBytes, err := json.Marshal(map[string]interface{}{k: v}); err == nil {
      // do a test unmarshal
      if json.Unmarshal(singleFieldBytes, testObjectMeta) == nil {
        // if that succeeds, unmarshal for real
        json.Unmarshal(singleFieldBytes, accumulatedObjectMeta)
      }
    }
  }
