...
func Coerce(pth *field.Path, obj interface{}, s *structuralschema.Structural, isResourceRoot, dropInvalidFields bool) *exceptions.Error {
  if isResourceRoot {
    if s == nil {
...
func (c *coercer) coerce(pth *field.Path, x interface{}, s *structuralschema.Structural) *exceptions.Error {
  if s == nil {
    return nil
  }
  switch x := x.(type) {
      ...
      if s.XEmbeddedResource {
        ...
        case "apiVersion", "kind":
          if _, ok := v.(string); !ok && c.dropInvalidFields {
            ...
            return exceptions.Invalid(pth.Child(k), v, "must be a string")
          }
        case "metadata":
          meta, found, err := GetObjectMeta(x, c.dropInvalidFields)
          if err != nil {
            if !c.dropInvalidFields {
              return exceptions.Invalid(pth.Child("metadata"), v, err.Error())
            }
