...
import (
  ...
  schemaobjectmeta "github.com/nrc-no/core/apps/api/pkg/server/extensions-apiserver/schema/objectmeta"
  "github.com/nrc-no/core/apps/api/pkg/server/extensions-apiserver/schema/pruning"
...
func ValidateDefaults(pth *field.Path, s *structuralschema.Structural, isResourceRoot, requirePrunedDefaults bool) (exceptions.ErrorList, error) {
...
func validate(pth *field.Path, s *structuralschema.Structural, rootSchema *structuralschema.Structural, f SurroundingObjectFunc, insideMeta, requirePrunedDefaults bool) (exceptions.ErrorList, error) {
  if s == nil {
    return nil, nil
  }
  ...
  if s.Default.Object != nil {
    ...
    if insideMeta {
      ...
      if err := schemaobjectmeta.Coerce(nil, obj, rootSchema, true, false); err != nil {
        allErrs = append(allErrs, exceptions.Invalid(pth.Child("default"), s.Default.Object, fmt.Sprintf("must result in valid metadata: %v", err)))
      } else if errs := schemaobjectmeta.Validate(nil, obj, rootSchema, true); len(errs) > 0 {
        allErrs = append(allErrs, exceptions.Invalid(pth.Child("default"), s.Default.Object, fmt.Sprintf("must result in valid metadata: %v", errs.ToAggregate())))
      ...
      if requirePrunedDefaults {
        ...
        if !reflect.DeepEqual(pruned, s.Default.Object) {
          allErrs = append(allErrs, exceptions.Invalid(pth.Child("default"), s.Default.Object, "must not have unknown fields"))
        }
      }
