...
import (
  "encoding/json"
  "github.com/nrc-no/core/apps/api/pkg/server/extensions-apiserver/schema"
...
func ValidateListSetsAndMaps(fldPath *field.Path, s *schema.Structural, obj map[string]interface{}) exceptions.ErrorList {
  if s == nil || obj == nil {
    return nil
  }
...
func validationListSetAndMaps(fldPath *field.Path, s *schema.Structural, obj interface{}) exceptions.ErrorList {
  switch obj := obj.(type) {
  case []interface{}:
...
func validateListSetsAndMapsArray(fldPath *field.Path, s *schema.Structural, obj []interface{}) exceptions.ErrorList {
  var errs exceptions.ErrorList
    ...
    case "set":
      ...
      if err != nil {
        ...
        for _, i := range nonUnique {
          errs = append(errs, exceptions.Duplicate(fldPath.Index(i), obj[i]))
        }
      }
    case "map":
...
func validateListSet(fldPath *field.Path, obj []interface{}) ([]int, *exceptions.Error) {
  if len(obj) <= 1 {
    return nil, nil
  }
  ...
  for i, x := range obj {
    ...
    case map[string]interface{}, []interface{}:
      ...
      if err != nil {
        return nil, exceptions.Invalid(fldPath.Index(i), x, "internal error")
      }
      s := string(bs)
...
func validateListMap(fldPath *field.Path, s *schema.Structural, obj []interface{}) exceptions.ErrorList {
  // only allow nil and objects
  for i, x := range obj {
    if _, ok := x.(map[string]interface{}); x != nil && !ok {
      return exceptions.ErrorList{exceptions.Invalid(fldPath.Index(i), x, "must be an object for an array of list-type map")}
    }
  }
  ...
  if len(s.XListMapKeys) == 1 {
    ...
    if err != nil {
      return exceptions.ErrorList{err}
    }
    ...
    for _, i := range nonUnique {
      ...
      case unspecifiedKeyValue{}:
        errs = append(errs, exceptions.Duplicate(fldPath.Index(i), map[string]interface{}{}))
      default:
        errs = append(errs, exceptions.Duplicate(fldPath.Index(i), map[string]interface{}{keyField: keys[i]}))
      }
    }
  ...
  var errs exceptions.ErrorList
  for _, i := range nonUnique {
    errs = append(errs, exceptions.Duplicate(fldPath.Index(i), keys[i]))
  }
