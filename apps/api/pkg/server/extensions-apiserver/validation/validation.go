...
func ValidateCustomResource(fldPath *field.Path, customResource interface{}, validator *validate.SchemaValidator) exceptions.ErrorList {
	if validator == nil {
		return nil
	}
	...
	for _, err := range result.Errors {
		...
		case *openapierrors.Validation:
			...
			case openapierrors.RequiredFailCode:
				allErrs = append(allErrs, exceptions.Required(errPath, ""))
			...
			case openapierrors.EnumFailCode:
				...
				allErrs = append(allErrs, exceptions.NotSupported(errPath, err.Value, values))
			...
			default:
				...
				allErrs = append(allErrs, exceptions.Invalid(errPath, value, err.Error()))
			}

		default:
			allErrs = append(allErrs, exceptions.Invalid(fldPath, "", err.Error()))
		}
	}
	return allErrs
}
