...
import (
  ...
  apiservervalidation "github.com/nrc-no/core/apps/api/pkg/server/extensions-apiserver/validation"
  "github.com/nrc-no/core/apps/api/pkg/util/exceptions"
  validation2 "github.com/nrc-no/core/apps/api/pkg/util/validation"
...
func ValidateCustomResourceDefinition(obj *apiextensions.CustomResourceDefinition, requestGV schema.GroupVersion) exceptions.ErrorList {
  ...
  allErrs := validation.ValidateObjectMeta(&obj.ObjectMeta, false, nameValidationFn, field.NewPath("metadata"))
...
func ValidateCustomResourceDefinitionUpdate(obj, oldObj *apiextensions.CustomResourceDefinition, requestGV schema.GroupVersion) exceptions.ErrorList {
  ...
  allErrs := validation.ValidateObjectMetaUpdate(&obj.ObjectMeta, &oldObj.ObjectMeta, field.NewPath("metadata"))
...
func ValidateCustomResourceDefinitionStoredVersions(storedVersions []string, versions []apiextensions.CustomResourceDefinitionVersion, fldPath *field.Path) exceptions.ErrorList {
  if len(storedVersions) == 0 {
    return exceptions.ErrorList{exceptions.Invalid(fldPath, storedVersions, "must have at least one stored version")}
  }
  ...
  for _, v := range versions {
    ...
    if v.Storage && !ok {
      allErrs = append(allErrs, exceptions.Invalid(fldPath, v, "must have the storage version "+v.Name))
    }
    if ok {
  ...
  for v, i := range storedVersionsMap {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Index(i), v, "must appear in spec.versions"))
  }
...
func ValidateUpdateCustomResourceDefinitionStatus(obj, oldObj *apiextensions.CustomResourceDefinition) exceptions.ErrorList {
  allErrs := validation.ValidateObjectMetaUpdate(&obj.ObjectMeta, &oldObj.ObjectMeta, field.NewPath("metadata"))
  allErrs = append(allErrs, ValidateCustomResourceDefinitionStatus(&obj.Status, field.NewPath("status"))...)
  return allErrs
}
...
func validateCustomResourceDefinitionVersion(version *apiextensions.CustomResourceDefinitionVersion, fldPath *field.Path, statusEnabled bool, opts validationOptions) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  for _, err := range validateDeprecationWarning(version.Deprecated, version.DeprecationWarning) {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("deprecationWarning"), version.DeprecationWarning, err))
  }
...
func validateCustomResourceDefinitionSpec(spec *apiextensions.CustomResourceDefinitionSpec, opts validationOptions, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}

  if len(spec.Group) == 0 {
    allErrs = append(allErrs, exceptions.Required(fldPath.Child("group"), ""))
  } else if errs := validation2.IsDNS1123Subdomain(spec.Group); len(errs) > 0 {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("group"), spec.Group, strings.Join(errs, ",")))
  } else if len(strings.Split(spec.Group, ".")) < 2 {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("group"), spec.Group, "should be a domain with at least one dot"))
  }
    ...
    if spec.Validation == nil || spec.Validation.OpenAPIV3Schema == nil {
      for i, v := range spec.Versions {
        if v.Schema == nil || v.Schema.OpenAPIV3Schema == nil {
          allErrs = append(allErrs, exceptions.Required(fldPath.Child("versions").Index(i).Child("schema").Child("openAPIV3Schema"), "schemas are required"))
        }
      }
    }
    ...
    if spec.Validation == nil || spec.Validation.OpenAPIV3Schema == nil {
      for i, v := range spec.Versions {
        ...
        if v.Served && (v.Schema == nil || v.Schema.OpenAPIV3Schema == nil) {
          allErrs = append(allErrs, exceptions.Required(schemaPath, "because otherwise all fields are pruned"))
        }
      }
    }
  }
  if opts.allowDefaults && specHasDefaults(spec) {
    ...
    if spec.PreserveUnknownFields == nil || *spec.PreserveUnknownFields == true {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("preserveUnknownFields"), true, "must be false in order to use defaults in the schema"))
    }
  }
  ...
  for i, version := range spec.Versions {
    ...
    if errs := validation2.IsDNS1035Label(version.Name); len(errs) > 0 {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("versions").Index(i).Child("name"), spec.Versions[i].Name, strings.Join(errs, ",")))
    }
  ...
  if spec.Validation != nil && hasPerVersionSchema(spec.Versions) {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("validation"), "top-level and per-version schemas are mutually exclusive"))
  }
  if spec.Subresources != nil && hasPerVersionSubresources(spec.Versions) {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("subresources"), "top-level and per-version subresources are mutually exclusive"))
  }
  if len(spec.AdditionalPrinterColumns) > 0 && hasPerVersionColumns(spec.Versions) {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("additionalPrinterColumns"), "top-level and per-version additionalPrinterColumns are mutually exclusive"))
  }
  ...
  if hasIdenticalPerVersionSchema(spec.Versions) {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("versions"), spec.Versions, "per-version schemas may not all be set to identical values (top-level validation should be used instead)"))
  }
  if hasIdenticalPerVersionSubresources(spec.Versions) {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("versions"), spec.Versions, "per-version subresources may not all be set to identical values (top-level subresources should be used instead)"))
  }
  if hasIdenticalPerVersionColumns(spec.Versions) {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("versions"), spec.Versions, "per-version additionalPrinterColumns may not all be set to identical values (top-level additionalPrinterColumns should be used instead)"))
  }

  if !uniqueNames {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("versions"), spec.Versions, "must contain unique version names"))
  }
  if storageFlagCount != 1 {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("versions"), spec.Versions, "must have exactly one version marked as storage version"))
  }
  if len(spec.Version) != 0 {
    if errs := validation2.IsDNS1035Label(spec.Version); len(errs) > 0 {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("version"), spec.Version, strings.Join(errs, ",")))
    }
    if len(spec.Versions) >= 1 && spec.Versions[0].Name != spec.Version {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("version"), spec.Version, "must match the first version in spec.versions"))
    }
  }

  // in addition to the basic name restrictions, some names are required for spec, but not for status
  if len(spec.Names.Plural) == 0 {
    allErrs = append(allErrs, exceptions.Required(fldPath.Child("names", "plural"), ""))
  }
  if len(spec.Names.Singular) == 0 {
    allErrs = append(allErrs, exceptions.Required(fldPath.Child("names", "singular"), ""))
  }
  if len(spec.Names.Kind) == 0 {
    allErrs = append(allErrs, exceptions.Required(fldPath.Child("names", "kind"), ""))
  }
  if len(spec.Names.ListKind) == 0 {
    allErrs = append(allErrs, exceptions.Required(fldPath.Child("names", "listKind"), ""))
  }
  ...
  if (spec.Conversion != nil && spec.Conversion.Strategy != apiextensions.NoneConverter) && (spec.PreserveUnknownFields == nil || *spec.PreserveUnknownFields) {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("conversion").Child("strategy"), spec.Conversion.Strategy, "must be None if spec.preserveUnknownFields is true"))
  }
...
func validateEnumStrings(fldPath *field.Path, value string, accepted []string, required bool) exceptions.ErrorList {
  if value == "" {
    if required {
      return exceptions.ErrorList{exceptions.Required(fldPath, "")}
    }
  ...
  return exceptions.ErrorList{exceptions.NotSupported(fldPath, value, accepted)}
}
...
var acceptedConversionReviewVersions = sets.NewString(apiextensions.SchemeGroupVersion.Version)

func isAcceptedConversionReviewVersion(v string) bool {
  return acceptedConversionReviewVersions.Has(v)
}

func validateConversionReviewVersions(versions []string, requireRecognizedVersion bool, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  if len(versions) < 1 {
    allErrs = append(allErrs, exceptions.Required(fldPath, ""))
  } else {
    seen := map[string]bool{}
    hasAcceptedVersion := false
    for i, v := range versions {
      if seen[v] {
        allErrs = append(allErrs, exceptions.Invalid(fldPath.Index(i), v, "duplicate version"))
        continue
      }
      seen[v] = true
      for _, errString := range validation2.IsDNS1035Label(v) {
        allErrs = append(allErrs, exceptions.Invalid(fldPath.Index(i), v, errString))
      }
    ...
    if requireRecognizedVersion && !hasAcceptedVersion {
      allErrs = append(allErrs, exceptions.Invalid(
        fldPath, versions,
...
func ValidateCustomResourceConversion(conversion *apiextensions.CustomResourceConversion, fldPath *field.Path) exceptions.ErrorList {
  return validateCustomResourceConversion(conversion, true, fldPath)
}

func validateCustomResourceConversion(conversion *apiextensions.CustomResourceConversion, requireRecognizedVersion bool, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  if conversion == nil {
    return allErrs
  }
  ...
  if conversion.Strategy == apiextensions.WebhookConverter {
    if conversion.WebhookClientConfig == nil {
      allErrs = append(allErrs, exceptions.Required(fldPath.Child("webhookClientConfig"), "required when strategy is set to Webhook"))
    } else {
      cc := conversion.WebhookClientConfig
      switch {
      case (cc.URL == nil) == (cc.Service == nil):
        allErrs = append(allErrs, exceptions.Required(fldPath.Child("webhookClientConfig"), "exactly one of url or service is required"))
      case cc.URL != nil:
    ...
    if conversion.WebhookClientConfig != nil {
      allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("webhookClientConfig"), "should not be set when strategy is not set to Webhook"))
    }
    if len(conversion.ConversionReviewVersions) > 0 {
      allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("conversionReviewVersions"), "should not be set when strategy is not set to Webhook"))
    }
  }
  return allErrs
}
...
func validateCustomResourceDefinitionSpecUpdate(spec, oldSpec *apiextensions.CustomResourceDefinitionSpec, opts validationOptions, fldPath *field.Path) exceptions.ErrorList {
  allErrs := validateCustomResourceDefinitionSpec(spec, opts, fldPath)

  if opts.requireImmutableNames {
    // these effect the storage and cannot be changed therefore
    allErrs = append(allErrs, validation.ValidateImmutableField(spec.Scope, oldSpec.Scope, fldPath.Child("scope"))...)
    allErrs = append(allErrs, validation.ValidateImmutableField(spec.Names.Kind, oldSpec.Names.Kind, fldPath.Child("names", "kind"))...)
  }

  // these affects the resource name, which is always immutable, so this can't be updated.
  allErrs = append(allErrs, validation.ValidateImmutableField(spec.Group, oldSpec.Group, fldPath.Child("group"))...)
  allErrs = append(allErrs, validation.ValidateImmutableField(spec.Names.Plural, oldSpec.Names.Plural, fldPath.Child("names", "plural"))...)
...
func hasIdenticalPerVersionSchema(versions []apiextensions.CustomResourceDefinitionVersion) bool {
  ...
  for _, v := range versions {
    if v.Schema == nil || !equality.Semantic.DeepEqual(v.Schema, value) {
      return false
    }
  }
  return true
...
func hasIdenticalPerVersionSubresources(versions []apiextensions.CustomResourceDefinitionVersion) bool {
  ...
  for _, v := range versions {
    if v.Subresources == nil || !equality.Semantic.DeepEqual(v.Subresources, value) {
      return false
    }
  }
  return true
...
func hasIdenticalPerVersionColumns(versions []apiextensions.CustomResourceDefinitionVersion) bool {
  ...
  for _, v := range versions {
    if len(v.AdditionalPrinterColumns) == 0 || !equality.Semantic.DeepEqual(v.AdditionalPrinterColumns, value) {
      return false
    }
  }
  return true
...
func ValidateCustomResourceDefinitionStatus(status *apiextensions.CustomResourceDefinitionStatus, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
...
func ValidateCustomResourceDefinitionNames(names *apiextensions.CustomResourceDefinitionNames, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  if errs := validation2.IsDNS1035Label(names.Plural); len(names.Plural) > 0 && len(errs) > 0 {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("plural"), names.Plural, strings.Join(errs, ",")))
  }
  if errs := validation2.IsDNS1035Label(names.Singular); len(names.Singular) > 0 && len(errs) > 0 {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("singular"), names.Singular, strings.Join(errs, ",")))
  }
  if errs := validation2.IsDNS1035Label(strings.ToLower(names.Kind)); len(names.Kind) > 0 && len(errs) > 0 {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("kind"), names.Kind, "may have mixed case, but should otherwise match: "+strings.Join(errs, ",")))
  }
  if errs := validation2.IsDNS1035Label(strings.ToLower(names.ListKind)); len(names.ListKind) > 0 && len(errs) > 0 {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("listKind"), names.ListKind, "may have mixed case, but should otherwise match: "+strings.Join(errs, ",")))
  }

  for i, shortName := range names.ShortNames {
    if errs := validation2.IsDNS1035Label(shortName); len(errs) > 0 {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("shortNames").Index(i), shortName, strings.Join(errs, ",")))
    }
  }

  // kind and listKind may not be the same or parsing become ambiguous
  if len(names.Kind) > 0 && names.Kind == names.ListKind {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("listKind"), names.ListKind, "kind and listKind may not be the same"))
  }

  for i, category := range names.Categories {
    if errs := validation2.IsDNS1035Label(category); len(errs) > 0 {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("categories").Index(i), category, strings.Join(errs, ",")))
    }
  }
...
func ValidateCustomResourceColumnDefinition(col *apiextensions.CustomResourceColumnDefinition, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}

  if len(col.Name) == 0 {
    allErrs = append(allErrs, exceptions.Required(fldPath.Child("name"), ""))
  }

  if len(col.Type) == 0 {
    allErrs = append(allErrs, exceptions.Required(fldPath.Child("type"), fmt.Sprintf("must be one of %s", strings.Join(printerColumnDatatypes.List(), ","))))
  } else if !printerColumnDatatypes.Has(col.Type) {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("type"), col.Type, fmt.Sprintf("must be one of %s", strings.Join(printerColumnDatatypes.List(), ","))))
  }

  if len(col.Format) > 0 && !customResourceColumnDefinitionFormats.Has(col.Format) {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("format"), col.Format, fmt.Sprintf("must be one of %s", strings.Join(customResourceColumnDefinitionFormats.List(), ","))))
  }

  if len(col.JSONPath) == 0 {
    allErrs = append(allErrs, exceptions.Required(fldPath.Child("JSONPath"), ""))
...
type specStandardValidator interface {
  validate(spec *apiextensions.JSONSchemaProps, fldPath *field.Path) exceptions.ErrorList
...
func validateCustomResourceDefinitionValidation(customResourceValidation *apiextensions.CustomResourceValidation, statusSubresourceEnabled bool, opts validationOptions, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  ...
  if schema := customResourceValidation.OpenAPIV3Schema; schema != nil {
      ...
      for i := 0; i < v.NumField(); i++ {
        ...
        if fieldName == "Type" {
          if schema.Type != "object" {
            allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("openAPIV3Schema.type"), schema.Type, fmt.Sprintf(`only "object" is allowed as the type at the root of the schema if the status subresource is enabled`)))
            break
          }
          continue
        }
        ...
        if !allowedAtRootSchema(fieldName) {
          allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("openAPIV3Schema"), *schema, fmt.Sprintf(`only %v fields are allowed at the root of the schema if the status subresource is enabled`, allowedFieldsAtRootSchema)))
          break
        }
      }
    }
    ...
    if schema.Nullable {
      allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("openAPIV3Schema.nullable"), fmt.Sprintf(`nullable cannot be true at the root`)))
    }
    ...
    if opts.requireStructuralSchema {
      if ss, err := schema2.NewStructural(schema); err != nil {
        ...
        if len(allErrs) == 0 {
          allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("openAPIV3Schema"), "", err.Error()))
        }
  ...
  if len(allErrs) == 0 {
    if _, _, err := apiservervalidation.NewSchemaValidator(customResourceValidation); err != nil {
      allErrs = append(allErrs, exceptions.Invalid(fldPath, "", fmt.Sprintf("error building validator: %v", err)))
    }
  }
  return allErrs
}
...
func ValidateCustomResourceDefinitionOpenAPISchema(schema *apiextensions.JSONSchemaProps, fldPath *field.Path, ssv specStandardValidator, isRoot bool, opts *validationOptions) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  ...
  if schema.UniqueItems == true {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("uniqueItems"), "uniqueItems cannot be set to true since the runtime complexity becomes quadratic"))
  }
  ...
  if schema.AdditionalProperties != nil {
    if len(schema.Properties) != 0 {
      if schema.AdditionalProperties.Allows == false || schema.AdditionalProperties.Schema != nil {
        allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("additionalProperties"), "additionalProperties and properties are mutual exclusive"))
      }
    }
  ...
  if schema.XPreserveUnknownFields != nil && *schema.XPreserveUnknownFields == false {
    allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("x-kubernetes-preserve-unknown-fields"), *schema.XPreserveUnknownFields, "must be true or undefined"))
  }

  if schema.XMapType != nil && schema.Type != "object" {
    if len(schema.Type) == 0 {
      allErrs = append(allErrs, exceptions.Required(fldPath.Child("type"), "must be object if x-kubernetes-map-type is specified"))
    } else {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("type"), schema.Type, "must be object if x-kubernetes-map-type is specified"))
    }
  }
  ...
  if schema.XListType != nil && schema.Type != "array" {
    if len(schema.Type) == 0 {
      allErrs = append(allErrs, exceptions.Required(fldPath.Child("type"), "must be array if x-kubernetes-list-type is specified"))
    } else {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("type"), schema.Type, "must be array if x-kubernetes-list-type is specified"))
    }
    ...
    case "array":
      if is.XListType != nil && *is.XListType != "atomic" { // atomic is the implicit default behaviour if unset, hence != atomic is wrong
        allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("items").Child("x-kubernetes-list-type"), is.XListType, "must be atomic as item of a list with x-kubernetes-list-type=set"))
      }
    case "object":
      if is.XMapType == nil || *is.XMapType != "atomic" { // granular is the implicit default behaviour if unset, hence nil and != atomic are wrong
        allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("items").Child("x-kubernetes-map-type"), is.XListType, "must be atomic as item of a list with x-kubernetes-list-type=set"))
      }
    }
  }
  ...
  if len(schema.XListMapKeys) > 0 {
    if schema.XListType == nil {
      allErrs = append(allErrs, exceptions.Required(fldPath.Child("x-kubernetes-list-type"), "must be map if x-kubernetes-list-map-keys is non-empty"))
    } else if *schema.XListType != "map" {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("x-kubernetes-list-type"), *schema.XListType, "must be map if x-kubernetes-list-map-keys is non-empty"))
    }
  }

  if schema.XListType != nil && *schema.XListType == "map" {
    if len(schema.XListMapKeys) == 0 {
      allErrs = append(allErrs, exceptions.Required(fldPath.Child("x-kubernetes-list-map-keys"), "must not be empty if x-kubernetes-list-type is map"))
    }

    if schema.Items == nil {
      allErrs = append(allErrs, exceptions.Required(fldPath.Child("items"), "must have a schema if x-kubernetes-list-type is map"))
    }

    if schema.Items != nil && schema.Items.Schema == nil {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("items"), schema.Items, "must only have a single schema if x-kubernetes-list-type is map"))
    }

    if schema.Items != nil && schema.Items.Schema != nil && schema.Items.Schema.Type != "object" {
      allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("items").Child("type"), schema.Items.Schema.Type, "must be object if parent array's x-kubernetes-list-type is map"))
    }
      ...
      for _, k := range schema.XListMapKeys {
        if s, ok := schema.Items.Schema.Properties[k]; ok {
          if s.Type == "array" || s.Type == "object" {
            allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("items").Child("properties").Key(k).Child("type"), schema.Items.Schema.Type, "must be a scalar type if parent array's x-kubernetes-list-type is map"))
          }
        } else {
          allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("x-kubernetes-list-map-keys"), schema.XListMapKeys, "entries must all be names of item properties"))
        }
        if _, ok := keys[k]; ok {
          allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("x-kubernetes-list-map-keys"), schema.XListMapKeys, "must not contain duplicate entries"))
        }
        keys[k] = struct{}{}
      }
    }
...
func validateMapListKeysMapSet(schema *apiextensions.JSONSchemaProps, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  ...
  if schema.Items.Schema.Nullable {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("items").Child("nullable"), "cannot be nullable when x-kubernetes-list-type is "+*schema.XListType))
  }
  ...
  case "map":
    ...
    for _, k := range schema.XListMapKeys {
      ...
      if isRequired[k] == false && obj.Default == nil {
        allErrs = append(allErrs, exceptions.Required(fldPath.Child("items").Child("properties").Key(k).Child("default"), "this property is in x-kubernetes-list-map-keys, so it must have a default or be a required property"))
      }

      if obj.Nullable {
        allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("items").Child("properties").Key(k).Child("nullable"), "this property is in x-kubernetes-list-map-keys, so it cannot be nullable"))
      }
    }
  case "set":
...
func (v *specStandardValidatorV3) validate(schema *apiextensions.JSONSchemaProps, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  ...
  if v.requireValidPropertyType && len(schema.Type) > 0 && !openapiV3Types.Has(schema.Type) {
    allErrs = append(allErrs, exceptions.NotSupported(fldPath.Child("type"), schema.Type, openapiV3Types.List()))
  }
  ...
  if schema.Default != nil && !v.allowDefaults {
    ...
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("default"), detail))
  }

  if schema.ID != "" {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("id"), "id is not supported"))
  }

  if schema.AdditionalItems != nil {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("additionalItems"), "additionalItems is not supported"))
  }

  if len(schema.PatternProperties) != 0 {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("patternProperties"), "patternProperties is not supported"))
  }

  if len(schema.Definitions) != 0 {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("definitions"), "definitions is not supported"))
  }

  if schema.Dependencies != nil {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("dependencies"), "dependencies is not supported"))
  }

  if schema.Ref != nil {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("$ref"), "$ref is not supported"))
  }

  if schema.Type == "null" {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("type"), "type cannot be set to null, use nullable as an alternative"))
  }

  if schema.Items != nil && len(schema.Items.JSONSchemas) != 0 {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("items"), "items must be a schema object and not an array"))
  }

  if v.isInsideResourceMeta && schema.XEmbeddedResource {
    allErrs = append(allErrs, exceptions.Forbidden(fldPath.Child("x-kubernetes-embedded-resource"), "must not be used inside of resource meta"))
  }
...
func ValidateCustomResourceDefinitionSubresources(subresources *apiextensions.CustomResourceSubresources, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}
  ...
  if subresources.Scale != nil {
    if len(subresources.Scale.SpecReplicasPath) == 0 {
      allErrs = append(allErrs, exceptions.Required(fldPath.Child("scale.specReplicasPath"), ""))
    } else {
      ...
      if errs := validateSimpleJSONPath(subresources.Scale.SpecReplicasPath, fldPath.Child("scale.specReplicasPath")); len(errs) > 0 {
        ...
        allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("scale.specReplicasPath"), subresources.Scale.SpecReplicasPath, "should be a json path under .spec"))
      }
    }

    if len(subresources.Scale.StatusReplicasPath) == 0 {
      allErrs = append(allErrs, exceptions.Required(fldPath.Child("scale.statusReplicasPath"), ""))
    } else {
      ...
      if errs := validateSimpleJSONPath(subresources.Scale.StatusReplicasPath, fldPath.Child("scale.statusReplicasPath")); len(errs) > 0 {
        ...
        allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("scale.statusReplicasPath"), subresources.Scale.StatusReplicasPath, "should be a json path under .status"))
      }
    }
    ...
    if subresources.Scale.LabelSelectorPath != nil && len(*subresources.Scale.LabelSelectorPath) > 0 {
      if errs := validateSimpleJSONPath(*subresources.Scale.LabelSelectorPath, fldPath.Child("scale.labelSelectorPath")); len(errs) > 0 {
        ...
        allErrs = append(allErrs, exceptions.Invalid(fldPath.Child("scale.labelSelectorPath"), subresources.Scale.LabelSelectorPath, "should be a json path under either .spec or .status"))
      }
    }
  }
...
func validateSimpleJSONPath(s string, fldPath *field.Path) exceptions.ErrorList {
  allErrs := exceptions.ErrorList{}

  switch {
  case len(s) == 0:
    allErrs = append(allErrs, exceptions.Invalid(fldPath, s, "must not be empty"))
  case s[0] != '.':
    allErrs = append(allErrs, exceptions.Invalid(fldPath, s, "must be a simple json path starting with ."))
  case s != ".":
    if cs := strings.Split(s[1:], "."); len(cs) < 1 {
      allErrs = append(allErrs, exceptions.Invalid(fldPath, s, "must be a json path in the dot notation"))
    }
  }
...
func validateAPIApproval(newCRD, oldCRD *apiextensions.CustomResourceDefinition, requestGV schema.GroupVersion) exceptions.ErrorList {
  ...
  case apihelpers.APIApprovalInvalid:
    return exceptions.ErrorList{exceptions.Invalid(field.NewPath("metadata", "annotations").Key(apiextensionsv1beta1.KubeAPIApprovedAnnotation), newCRD.Annotations[apiextensionsv1beta1.KubeAPIApprovedAnnotation], reason)}
  case apihelpers.APIApprovalMissing:
    return exceptions.ErrorList{exceptions.Required(field.NewPath("metadata", "annotations").Key(apiextensionsv1beta1.KubeAPIApprovedAnnotation), reason)}
...
func validatePreserveUnknownFields(crd, oldCRD *apiextensions.CustomResourceDefinition, requestGV schema.GroupVersion) exceptions.ErrorList {
  if requestGV == apiextensionsv1beta1.SchemeGroupVersion {
    // no-op for compatibility with v1beta1
    return nil
  }
  ...
  if crd != nil && crd.Spec.PreserveUnknownFields != nil && *crd.Spec.PreserveUnknownFields {
    ...
    errs = append(errs, exceptions.Invalid(field.NewPath("spec").Child("preserveUnknownFields"), crd.Spec.PreserveUnknownFields, "cannot set to true, set x-preserve-unknown-fields to true in spec.versions[*].schema instead"))
  }
  return errs
}
