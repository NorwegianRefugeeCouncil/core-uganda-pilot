...
ADD --chown=node:node frontend/apps/pwa/package.json ./apps/pwa/package.json
...
ADD --chown=nginx:nginx configs/core-frontend/nginx.conf.template  /etc/nginx/templates/nginx.conf.template
ADD --chown=nginx:nginx configs/core-frontend/replace-env-vars.sh /docker-entrypoint.d/40-subst-on-assets.sh
COPY --from=build --chown=nginx:nginx /home/node/app/apps/pwa/build /usr/share/nginx/html
