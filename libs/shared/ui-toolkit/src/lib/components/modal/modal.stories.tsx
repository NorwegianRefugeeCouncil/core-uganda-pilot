...
const ModalStory = (props: React.PropsWithChildren<StoryProps>) => {
  ...
  return (
    <div className={'mb-2 border p-3'}>
      ...
      <Modal {...otherProps} show={show} onHide={() => setShow(false)}>
        {content(setShow)}
      </Modal>
      {children}
    </div>
  ...
  return (
    <>
      ...
      <ModalStory storyName={'Static Backdrop'} backdrop={'static'}>
        <p>
          A modal with a <code>static</code> backdrop. That means that a click
          on the backdrop will not close the modal
        </p>
      </ModalStory>
      <ModalStory
        storyName={'No Keyboard'}
        keyboard={false}
        backdrop={'static'}
      >
        <p>
          A modal with a <code>keyboard=false</code> and{' '}
          <code>backdrop=static</code> backdrop. That means that pressing the{' '}
          <code>esc</code> key will not close the backdrop
        </p>
      </ModalStory>
