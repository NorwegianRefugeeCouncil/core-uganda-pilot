...
#OAUTH_CORE_ADMIN_REDIRECT_URI=$(createFileIfNotExists "${CREDS_DIR}/core/admin_api/oauth_redirect_uri" "echo -n http://localhost:3001")
#OAUTH_CORE_ADMIN_ISSUER=$(createFileIfNotExists "${CREDS_DIR}/core/admin_api/oauth_issuer" "echo -n http://localhost:9005")
...
#OAUTH_CORE_APP_REDIRECT_URI=$(createFileIfNotExists "${CREDS_DIR}/core/app_api/oauth_redirect_uri" "echo -n http://localhost:3000")
#OAUTH_CORE_APP_ISSUER=$(createFileIfNotExists "${CREDS_DIR}/core/app_api/oauth_issuer" "echo -n http://localhost:4444/")
...
#OAUTH_NRC_REDIRECT_URI=$(createFileIfNotExists "${CREDS_DIR}/nrc_idp/oauth_redirect_uri" "echo -n http://localhost:9002/login/oidc/callback")
#OAUTH_NRC_ISSUER=$(createFileIfNotExists "${CREDS_DIR}/nrc_idp/oauth_issuer" "echo -n http://localhost:9005")
...
#
#
#CA_CFG_FILE="${CREDS_DIR}/ca/openssl.cnf"
#CA_KEY_FILE="${CREDS_DIR}/ca/tls.key"
#CA_CSR_FILE="${CREDS_DIR}/ca/cert.csr"
#CA_CERT_FILE="${CREDS_DIR}/ca/tls.cert"
#CA_COMBINED_FILE="${CREDS_DIR}/ca/ca.p12"
#if [ ! -f "${CA_CFG_FILE}" ]; then
#  mkdir -p "$(dirname "${CA_CFG_FILE}")"
#  cat <<EOF > "${CA_CFG_FILE}"
#[req]
#default_bits = 2048
...
#createFileIfNotExists "${CA_CSR_FILE}" "openssl req -new -key ${CA_KEY_FILE} -config ${CA_CFG_FILE}"
#createFileIfNotExists "${CA_CERT_FILE}" "openssl x509 -in ${CA_CSR_FILE} -req -signkey ${CA_KEY_FILE} -days 365 -extensions v3_req -extfile ${CA_CFG_FILE}"
#createFileIfNotExists "${CA_COMBINED_FILE}" "openssl pkcs12 -export -nodes -in ${CA_CERT_FILE} -inkey ${CA_KEY_FILE} -passout pass:"
#
#function createCert() {
#  local DIR
#  local NAME
